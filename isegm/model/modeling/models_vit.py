...
class PatchEmbed(nn.Module):
    ...
    """
    def __init__(self, img_size=(448,448), patch_size=(16,16), in_chans=3, embed_dim=768,
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=(224,224), patch_size=(16, 16), in_chans=3, num_classes=1000, embed_dim=768,
        ...
        # self.cls_token = nn.Parameter(jittor.zeros(1, 1, embed_dim))
        # self.pos_embed = nn.Parameter(jittor.zeros(1, num_patches + 1, embed_dim)) # learnable positional embedding
        self.cls_token = jittor.zeros(1, 1, embed_dim)
        self.pos_embed = jittor.zeros(1, num_patches + 1, embed_dim) # learnable positional embedding
    ...
    def init_weights_from_pretrained(self, pretrained_path):
        if pretrained_path:
            # checkpoint = jittor.load(pretrained_path, map_location='cpu')
            checkpoint = jittor.load(pretrained_path)
            ...
            msg = self.load_state_dict(checkpoint_model)
            print(msg)

    def init_weights(self):

        w = self.patch_embed.proj.weight
        jittor.init.xavier_uniform_(w.view([w.shape[0], -1]))

        # Initialize cls_token and pos_embed with normal distribution
        jittor.normal(self.cls_token, std=.02)
        jittor.normal(self.pos_embed, std=.02)

        # Apply initialization to all layers in the model
    ...
    def _init_weights(self, m):
        if isinstance(m, nn.Linear):
            ...
            if m.bias is not None:
                jittor.init.constant_(m.bias, 0)
        elif isinstance(m, nn.LayerNorm):
            jittor.init.constant_(m.bias, 0)
            jittor.init.constant_(m.weight, 1.0)
    ...
    def shuffle(self, x):
        ...
        """
        B, N, C = x.shape
        noise = jittor.rand(B, N, device=x.device)
        ids_shuffle = jittor.argsort(noise, dim=1)
        ids_restore = jittor.argsort(ids_shuffle, dim=1)

        x_shuffle = jittor.gather(x, 1, index=ids_shuffle.unsqueeze(-1).repeat(1, 1, C))

        return x_shuffle, ids_restore

    def unshuffle(self, x, ids_restore):
        B, N, C = x.shape
        x_unshuffle = jittor.gather(x, 1, index=ids_restore.unsqueeze(-1).repeat(1, 1, C))
    ...
    def execute(self, x):
        ...
        x = jittor.concat((cls_token, x), dim=1)
