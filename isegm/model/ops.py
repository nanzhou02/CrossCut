...
class DistMaps(nn.Module):
    ...
    def get_coord_features(self, points, batchsize, rows, cols):
        if self.cpu_mode:
            ...
            coords = jittor.array(np.stack(coords, axis=0)).float()
        else:
            num_points = points.shape[1] // 2
            points = points.view(-1, points.size(2))
            points, points_order = jittor.split(points, [2, 1], dim=1)

            invalid_points = jittor.max(points, dim=1, keepdim=False) < 0
            # invalid_points = jittor.max(points, dim=1)[0] < 0

            row_array = jittor.arange(start=0, end=rows, step=1, dtype=jittor.float32)
            col_array = jittor.arange(start=0, end=cols, step=1, dtype=jittor.float32)

            coord_rows, coord_cols = jittor.meshgrid(row_array, col_array)
            coords = jittor.stack((coord_rows, coord_cols), dim=0).unsqueeze(0).repeat(points.size(0), 1, 1, 1)
...
class ScaleLayer(nn.Module):
    def __init__(self, init_value=1.0, lr_mult=1):
        ...
        self.scale = jittor.full((1,), init_value / lr_mult, dtype=jittor.float32)

    def execute(self, x):
        scale = jittor.abs(self.scale * self.lr_mult)
        return x * scale


class BatchImageNormalize:
    def __init__(self, mean, std, dtype=jittor.float):
        self.mean = jittor.array(mean, dtype=dtype)[None, :, None, None]
        self.std = jittor.array(std, dtype=dtype)[None, :, None, None]

    def __call__(self, tensor):
        tensor = tensor.clone()

        tensor = (tensor - self.mean) / self.std  # 直接进行广播计算
        return tensor
